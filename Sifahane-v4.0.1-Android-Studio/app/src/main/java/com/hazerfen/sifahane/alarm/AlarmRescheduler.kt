...
object AlarmRescheduler {
    suspend fun clearStaleAndRefresh(context: Context): Pair<Int, AlarmRefreshResult> {
        ...
        val removedMedicationGroups = AlarmScheduler.cancelAllRegisteredGroups(context)
        val removedAppointmentAlarms = AppointmentAlarmScheduler.cancelAllRegistered(context)
        val removedTestAlarm = if (AlarmScheduler.cancelTest(context)) 1 else 0
        AlarmActivity.closeActiveAlarmScreen()
        ...
        AlarmDeliveryGate.clearAll(context)
        // Eski bakım sürümlerinin teslim kapısı tercihleri de bir kez temizlenir.
        context.getSharedPreferences("alarm_delivery_v3353", Context.MODE_PRIVATE).edit().clear().apply()
        return (removedMedicationGroups + removedAppointmentAlarms + removedTestAlarm) to refreshAll(context)
    }
    ...
    suspend fun refreshAll(context: Context): AlarmRefreshResult {
            ...
            AlarmScheduler.medicationTimes(medication).forEach { scheduledTime ->
                ...
                if (AlarmScheduler.isMedicationValidOn(medication, plannedToday)) {
                    if (plannedToday > now) {
                        ...
                        val answered = latest?.action == DoseAction.TAKEN.storageValue || latest?.action == DoseAction.SKIPPED.storageValue
        ...
        val groups = pending.groupBy {
            Triple(it.kind, it.dose.profileId, it.triggerAt / 60_000L)
        }
