...
interface ProfileDao {
    ...
    @Query(
        """
        SELECT
            (SELECT COUNT(*) FROM medications WHERE profileId=:profileId) +
            (SELECT COUNT(*) FROM dose_logs WHERE profileId=:profileId) +
            (SELECT COUNT(*) FROM blood_pressure WHERE profileId=:profileId) +
            (SELECT COUNT(*) FROM blood_glucose WHERE profileId=:profileId) +
            (SELECT COUNT(*) FROM report_groups WHERE profileId=:profileId) +
            (SELECT COUNT(*) FROM appointments WHERE profileId=:profileId)
        """
    )
    suspend fun relatedRecordCount(profileId: Long): Int
...
interface MedicationDao {
    ...
    @Query("SELECT COUNT(*) FROM dose_logs WHERE medicationId=:medicationId")
    suspend fun doseLogCount(medicationId: Long): Int

    @Query("UPDATE medications SET active=0, archived=1 WHERE id=:medicationId")
    suspend fun archiveForHistory(medicationId: Long): Int
...
interface DoseLogDao {
    ...
    @Query("""
        ...
        WHERE medicationId=:medicationId AND scheduledDateTime=:scheduledDateTime
        ORDER BY timestamp DESC, id DESC
        LIMIT 1
    """)
    suspend fun latestForDose(medicationId: Long, scheduledDateTime: Long): DoseLog?

    @Query("""
        SELECT COUNT(*) FROM dose_logs
        WHERE medicationId=:medicationId
          AND scheduledDateTime=:scheduledDateTime
          AND action='ALINDI GERİ ALINDI'
          AND timestamp>=:takenTimestamp
    """)
    suspend fun undoCountAfter(
        medicationId: Long,
...
interface AppointmentDao {
    ...
    @Query("SELECT * FROM appointments WHERE profileId=:profileId ORDER BY appointmentDateTime ASC")
    suspend fun allForProfile(profileId: Long): List<Appointment>
