...
object AdminPinStore {
    ...
    /** Profile-scoped legacy PIN doğrulaması da aynı kalıcı deneme sınırını kullanır. */
    fun recordExternalFailure(context: Context) {
        val prefs = context.getSharedPreferences(PREF, Context.MODE_PRIVATE)
        recordFailure(prefs, FAILURES, LOCKOUT_UNTIL)
    }
    ...
    /**
     * Yalnız tamamlanamayan ilk kurulumun atomik geri alınmasında kullanılır.
     * Mevcut profil bulunan normal akışlarda çağrılmamalıdır.
     */
    fun clearForIncompleteFirstRun(context: Context) {
        val cleared = context.getSharedPreferences(PREF, Context.MODE_PRIVATE)
            .edit()
            .remove(PIN_HASH)
            .remove(INITIALIZED)
            .remove(FAILURES)
            .remove(LOCKOUT_UNTIL)
            .commit()
        check(cleared) { "Yarım kalan yönetici şifresi temizlenemedi." }
    }
