...
class AppointmentAlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val alarmIdentity = intent.getStringExtra("alarmIdentity")
            ?: "APPOINTMENT:$appointmentId:${intent.getIntExtra("reminderMinutes", 0)}"
        if (!AlarmDeliveryGate.claim(context, alarmIdentity)) return
        ...
        AlarmDeliveryGate.markHandled(context, alarmIdentity)
    }
}
